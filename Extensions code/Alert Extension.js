(function (Scratch) {
    ...
    class AlertExtension {
      ...
      // בלוק שמציג alert עם טקסט קבוע
      showAlert(args) {
        alert(args.TEXT);
      }
  
      // בלוק שמחזיר תשובת טקסט מהמשתמש
      promptText(args) {
        return prompt(args.MESSAGE || 'הכנס טקסט כאן:');
      }
  
      // בלוק שמחזיר true אם המשתמש לוחץ על "אישור", אחרת false
      confirmAction(args) {
        return confirm(args.MESSAGE || 'האם אתה בטוח?');
      }
  
      // בלוק מציג התראה עם כותרת, הודעה ומשך זמן
      ...
      // בלוק מציג התראה עם אייקון מותאם אישית
      WebExt_DisplayNotificationWithIcon(args) {
        new Notification(args.TITLE, {
          body: args.MESSAGE,
          icon: args.URL // כאן ניתן להכניס את ה-URL לאייקון
        });
        setTimeout(() => {
          // נסגר לאחר זמן מוגדר
        }, args.DURATION * 1000);
      ...
      // בלוק בודק אם אפשר להציג התראה
      WebExt_canDisplayNotification() {
        return Notification.permission === 'granted';
      }
  
      getInfo() {
        // הכנס את התמונה כאן בקוד Base64
